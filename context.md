...
A compact loss-versus-iteration chart floats immediately above the left edge of the control dock during playback only; the opening ready frame contains no chart or residual guide. The plot relies on its labeled `LOSS` and `ITERATION` axes without a redundant title. Its faint guide is the selected preset's actual deterministic loss history, while an accent trace and marker reveal in exact synchronization with the ball's current gradient-descent iteration. During Reset the trace rewinds with the ball and the complete chart dissolves away when the opening state is restored. The chart remains intentionally unboxed and subordinate to the main surface.
