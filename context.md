...
- One smooth 3D upward-opening paraboloid representing the loss landscape. In its principal coordinates the surface is the explicit positive quadratic `z = 0.5 λu u² + 0.5 λv v²`, with both eigenvalues positive and no cross term. The center is the unique minimum. The opening frame shows a finite rectangular parameter-domain patch, so its four corners and four smoothly curved boundary arcs read like the supplied reference without altering the positive curvature of the bowl. The same continuous surface expands beyond the scene during the camera move, making the browser viewport the visible clipping boundary in the top view.
- The experience uses a pure black background with no top header, logo strip, status strip, or header divider.
- The surface uses a continuous reference-matched height gradient: deep blue at the unique minimum, then royal blue, cyan, aqua, yellow-green, yellow, orange, and red at the highest elevations. A single antialiased WebGL surface renders the complete camera sequence with smooth per-vertex height color, normal-based lighting, and a real depth buffer. There is no longer a painter-sorted mesh or crossfade between separate surface representations, so near/far slopes remain correctly occluded at every camera angle. Domain expansion uses a delayed cubic progression and zoom uses a delayed eased progression, keeping the opening composition stable before the surface fills the top view. The offscreen surface buffer renders at full device-pixel ratio and composites crisply into the presentation canvas.
...
- Forward and reverse use the same depth-buffered geometry throughout. With no renderer handoff, alpha mask, or decorative analytical rim, there is no competing silhouette that can create a polygonal pop or transient boundary flash.
...
Each option includes its numeric learning rate. It also provides a clear play control that becomes `Reset` after a run; Reset returns to the ready frame and waits for a new Play. The selected option is unmistakable but visually quiet, using semantic color and type rather than a boxed selection. Controls live in a tightly fitted translucent glass panel with minimal unused space and remain anchored and stable while the scene animates. At viewport widths of 900px and below, the three presets remain in one horizontal row while Play/Reset becomes a compact centered 112px-wide, 44px-high control beneath them rather than a full-width bar.

A compact loss-versus-iteration chart floats immediately above the left edge of the control dock during playback only; the opening ready frame contains no chart. The plot relies on its labeled `LOSS` and `ITERATION` axes without a redundant title. No preview curve is shown: the axes appear first, then the selected preset's actual deterministic loss trace and marker reveal in exact synchronization with the ball's current gradient-descent iteration. During Reset the trace rewinds with the ball and the complete chart dissolves away when the opening state is restored. The chart remains intentionally unboxed and subordinate to the main surface, while its axes, labels, and active trace retain enough contrast to stay legible over the animated surface.
...
- Ball entrance: an explicit three-stage scale bounce within the existing 300 ms entrance window—grow to `1.08×`, rebound to `0.96×`, then settle at `1×`—with smooth easing between each stage.
